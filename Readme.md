...
* **慢速路徑 (Slow Track / 大腦皮層)：** 系統刻意引入 250 毫秒的生理延遲 (Somatic Delay)。主模型甦醒後，會優先讀取已被「數位化學物質」擾動的身體狀態，進而執行基於 OCC (Ortony, Clore, and Collins) 模型的高階認知評估與邏輯建構 (Analytic Reasoning)。
...
本研究提出「Zero-ENV」動態配置架構，徹底屏棄傳統硬編碼的 `.env` 檔案。所有的 LLM API Keys、OpenClaw 技能存取權杖與系統行為參數，皆透過前端「神經介面 (Neural Interface)」的圖形化設定精靈進行寫入，實現前後端分離的狀態持久化管理。
...
啟動後，請前往 `http://localhost:8080` 存取神經介面。
