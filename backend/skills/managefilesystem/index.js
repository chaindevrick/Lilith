...
const checkPathScope = (absPath) => {
    if (!absPath.startsWith(ROOT_DIR)) {
        return `[System Alert] 邊界限制：妳無法離開目錄 (${ROOT_DIR})。`;
    }
    return null;
};
...
const listProjectStructure = async (dir = '.', depth = 5) => {
...
const readCodeFile = async (relativePath) => {
...
const writeCodeFile = async (relativePath, newContent) => {
    const { absPath, finalPath } = await resolveSmartPath(relativePath);
    const scopeError = checkPathScope(absPath);
    if (scopeError) return scopeError;
...
export default {
    // 🌟 接收注入的 config
    run: async ({ action, path: targetPath, content, destPath }, config) => {
        switch (action) {
            case 'list': return await listProjectStructure(targetPath);
            case 'read': return await readCodeFile(targetPath);
            case 'write': return await writeCodeFile(targetPath, content);
            case 'move': return await moveFile(targetPath, destPath);
            case 'delete': return await deleteFile(targetPath);
            default: return `[System Error] 未知的 FS 動作: ${action}`;
        }
    }
};